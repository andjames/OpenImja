...
const validationRevision = "20260923-contextual-archive";
...
function renderPair(row, geometry) {
  ...
  document.querySelector("#pair-metrics").innerHTML = metric("OPTICAL AREA", numeric(row.sentinel2_area_km2, 3, " km²"), row.sentinel2_observed_at.slice(0, 10)) + metric("SAR AREA", numeric(row.sentinel1_area_km2, 3, " km²"), row.sentinel1_observed_at.slice(0, 10)) + metric("TIME SEPARATION", numeric(row.temporal_separation_hours, 1, " h"), `${numeric(row.temporal_separation_days, 2, " days")} · SAR relative to optical`) + metric("PAIR CONTEXT", row.pairing_class || "near_coincident", row.pairing_class === "near_coincident" ? "Eligible for direct temporal validation" : "Coverage/method context; not same-period validation") + metric("AREA DIFFERENCE", numeric(row.absolute_area_difference_km2, 3, " km²"), `${numeric(row.percentage_area_difference, 1, "%")} of optical area`) + metric("POLYGON IoU", numeric(row.iou, 3), "Intersection over union; descriptive only");
  document.querySelector("#pair-provenance").innerHTML = field("Optical product", row.sentinel2_product_id) + field("SAR product", row.sentinel1_product_id) + field("SAR state", `<span class="state state-${row.sentinel1_state}">${row.sentinel1_state || "unknown"}</span>`) + field("Pairing policy", `±${row.pairing_window_days || "—"} days retained; ±${row.strict_window_days || "—"} days near-coincident`) + field("Orbit / polarization", `${row.sentinel1_orbit_pass || "—"} / ${row.sentinel1_polarization || "—"}`) + field("SAR QA", row.sentinel1_quality_flags || "None") + field("Optical QA", row.sentinel2_quality_flags || "None") + field("Optical omission", numeric(row.optical_omission_fraction, 3)) + field("SAR commission", numeric(row.sar_commission_fraction, 3)) + field("Centroid displacement", numeric(row.centroid_displacement_degrees, 5, "°"));
}

async function loadValidation() {
  const [pairsResponse, geometryResponse] = await Promise.all([fetch(`${validationRoot}/s1-s2-pairs.csv?v=${validationRevision}`, {cache:"no-store"}), fetch(`${validationRoot}/s1-s2-pair-geometries.geojson?v=${validationRevision}`, {cache:"no-store"})]);
  ...
  const show = () => { const row = pairs[index]; label.textContent = `${index + 1} / ${pairs.length} · ${row.sentinel2_observed_at.slice(0,10)} optical / ${row.sentinel1_observed_at.slice(0,10)} SAR · ${row.pairing_class || "near_coincident"} · ${row.sentinel1_state || "unknown"}`; prev.disabled = index === 0; next.disabled = index === pairs.length - 1; renderPair(row, geometry); };
